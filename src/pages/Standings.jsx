...
export function Standings() {
  ...
  return (
    <div className="animate-fade-in py-12 px-4 max-w-5xl mx-auto">
      <h1 className="text-4xl md:text-5xl font-extrabold text-deep-navy mb-4 tracking-tight">{t('standings.season')} <span className="text-fuchsia-accent">{t('standings.season_year')}</span></h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-sm uppercase tracking-wider text-gray-500">
                ...
                <th className="p-6 font-bold text-center text-deep-navy">{t('standings.col_points')}</th>
              </tr>
            </thead>
            <tbody>
              {standingsData.map((row) => (
                <tr key={row.rank} className="border-b border-gray-50 hover:bg-gray-50 transition-colors bg-white">
                  ...
                  <td className="p-6 font-bold text-xl text-deep-navy">{row.team}</td>
