...
export function Events() {
  ...
  return (
    <div className="animate-fade-in py-12 px-4 max-w-4xl mx-auto">
      <h1 className="text-4xl md:text-5xl font-extrabold text-deep-navy mb-4 tracking-tight">{t('events_page.title_target')} <span className="text-fuchsia-accent">{t('events_page.title_accent')}</span></h1>
        ...
        {eventsData.map(event => {
          ...
          return (
            <Card key={event.id} className="flex flex-col md:flex-row overflow-hidden hover:shadow-lg transition-all duration-300">
               <div className="bg-fuchsia-accent text-white p-6 md:w-56 flex flex-col justify-center items-center text-center">
              ...
              <div className="p-8 md:p-10 flex-1 flex flex-col justify-center bg-white">
                <div className={`text-xs font-bold uppercase tracking-wider mb-3 ${event.type === 'match' ? 'text-fuchsia-accent' : 'text-blue-600'}`}>
                  {event.type === 'match' ? t('events_page.type_match') : t('events_page.type_training')}
                </div>
                <h3 className="text-3xl font-bold text-deep-navy mb-4">{event.title}</h3>
